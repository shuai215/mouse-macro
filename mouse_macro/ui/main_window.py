...
from mouse_macro.ui.overlay import NotificationOverlay
...
class MainWindow(FluentWindow):
    def __init__(self, controller: MacroController, locale: LocaleManager) -> None:
        ...
        self.signals.loop_progress.connect(self._on_loop_progress)
    ...
    def start_recording(self) -> None:
        ...
        self.overlay.notify(self.locale.tr("status_recording"))
        self._update_state()

    def stop_recording(self) -> None:
        count = self.controller.stop_recording()
        self.overlay.notify(
            self.locale.tr("status_idle")
            + f" ({self.locale.tr('label_actions', count)})"
        )
        self._show_info(
    ...
    def play(self) -> None:
        ...
        if not self.controller.play(loops, on_loop=self._on_loop):
            self._show_info(self.locale.tr("info_no_actions"), "")
        self._update_state()
    ...
    def _on_loop(self, current: int, total: int) -> None:
        self.signals.loop_progress.emit(current, total)

    def _on_loop_progress(self, current: int, total: int) -> None:
        self.overlay.show_persistent(f"Loop {current}/{total}")
    ...
    def _toggle_playing_from_ui_thread(self) -> None:
        if self.controller.is_playing:
            ...
            if not self.controller.play(
                self.loop_spin.value(), on_loop=self._on_loop
            ):
                self._show_info(self.locale.tr("info_no_actions"), "")
        self._update_state()
